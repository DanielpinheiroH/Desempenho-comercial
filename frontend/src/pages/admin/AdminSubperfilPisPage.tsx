...
function ordemMes(mes?: string) {
  ...
  const numeroMes = Number(getMesNumero(mes))

  if (!Number.isFinite(ano) || !Number.isFinite(numeroMes)) {
    return 0
  }

  return Number(`${ano}${String(numeroMes).padStart(2, "0")}`)
}

export default function AdminSubperfilPisPage() {
  ...
  const mesesDisponiveis = useMemo(() => {
    ...
    return Array.from(new Set(meses)).sort((a, b) => Number(b) - Number(a))
  }, [dadosDoSubperfil, anoSelecionado])

  const dadosSubperfil = useMemo(() => {
    const termo = normalizar(busca)

    return dadosDoSubperfil
      .filter((item) => {
      ...
      .sort((a, b) => ordemMes(b.mes_venda) - ordemMes(a.mes_venda))
  }, [dadosDoSubperfil, busca, anoSelecionado, mesSelecionado])

  const totalLiquido = dadosSubperfil.reduce(
    (acc, item) => acc + Number(item.valor_liquido || 0),
    0
  )

  const totalBruto = dadosSubperfil.reduce(
  ...
  return (
    <main className="min-h-screen space-y-6 bg-zinc-100 text-zinc-950">
      <section className="rounded-[2rem] bg-zinc-950 p-8 text-white">
        <button
          type="button"
          onClick={() => navigate(`/admin/subperfil/${encodeURIComponent(subperfilAtual)}`)}
        ...
        <p className="mt-2 text-zinc-300">
          Todos os PIs encontrados neste subperfil.
        </p>
      </section>

      <section className="grid gap-4 md:grid-cols-3">
        <KpiCard label="Total de PIs" value={String(dadosSubperfil.length)} />
      ...
      <section className="rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-sm">
        <div className="grid gap-3 xl:grid-cols-[160px_170px_1fr_auto]">
          ...
          <input
            ...
            placeholder="Buscar por PI, mês, executivo, diretoria, anunciante, agência..."
          />
...
function PiDetalhe({ pi, onClose }: { pi: Pi; onClose: () => void }) {
  return (
    <section className="rounded-[2rem] border border-red-200 bg-white p-5 shadow-sm">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <InfoCard label="Perfil" value={pi.perfil_anunciante} />
        <InfoCard label="Subperfil" value={pi.sub_perfil_anunciante} />
        ...
        <InfoCard label="Emissão/Recebimento PI" value={pi.data_emissao_recebimento_pi} />
      </div>
